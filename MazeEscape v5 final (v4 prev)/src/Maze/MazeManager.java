...
public class MazeManager {
    ...
    public void rotateCorridor(int rowId) {
        ...
        // Collect agents in the row using AuxLL
        AuxLL rowAgents = new AuxLL();
        for (int i = 0; i < agents.size(); i++) {
            Agent a = agents.get(i);
            if (a.getCurrentX() == rowId) {
                rowAgents.add(a.getId() + "," + a.getCurrentY());
            }
        }
        ...
        // Update grid with rotated tiles and clear hasAgent
        ...
        for (int i = 0; i < rowAgents.size(); i++) {
            String[] data = rowAgents.get(i).split(",");
            int agentId = Integer.parseInt(data[0]);
            int oldY = Integer.parseInt(data[1]);
            int newY = (oldY + 1) % width;
