...
public class MazeManager {
    ...
    public void moveAgent(Agent a, String direction) {
        if (a.isFinished()) {
            System.out.println("Agent " + a.getId() + " is finished and cannot move");
            return;
        }
    ...
    public void rotateCorridor(int rowId) {
        ...
        // Step 1: Shift tiles left
        MazeTile[] originalRow = new MazeTile[width];
        for (int y = 0; y < width; y++) {
            originalRow[y] = grid[rowId][y];
        }
        for (int y = 0; y < width - 1; y++) {
            grid[rowId][y] = originalRow[y + 1];
        }
        grid[rowId][width - 1] = originalRow[0];

        // Step 2: Clear hasAgent for the entire row
        for (int y = 0; y < width; y++) {
            grid[rowId][y].setHasAgent(false);
        }

        // Step 3: Move agents and update positions
        AgentList agents = getAgentList();
        for (int i = 0; i < agents.size(); i++) {
            Agent a = agents.get(i);
            if (a != null && a.getCurrentX() == rowId && !a.isFinished()) {
                int oldY = a.getCurrentY();
                int newY = (oldY - 1 + width) % width;

                // Clear old position
                grid[rowId][oldY].setHasAgent(false);

                // Update agent position
                a.setY(newY);

                // Update new position if valid
                if (grid[rowId][newY].isTraversable()) {
                    grid[rowId][newY].setHasAgent(true);
                    System.out.println("Agent " + a.getId() + " moved to (" + rowId + "," + newY + ") due to corridor rotation");
                } else {
                    System.out.println("Agent " + a.getId() + " stuck at invalid position after rotation");
                    a.setStuck(true); // <-- THIS WAS MISSING
                }
            }
        }
    }
